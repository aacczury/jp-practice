...
function playLine(L, ln) { stopAudio(); ytPause(); return rawPlay(L, ln); }  // single tap: interrupt, then play
...
function cleanupYT() { try { if (ytPlayer && ytPlayer.destroy) ytPlayer.destroy(); } catch (e) {} ytPlayer = null; ytPending = null; }
function ytSeek(sec) { stopAudio(); if (ytPlayer && ytPlayer.seekTo) { ytPlayer.seekTo(sec, true); ytPlayer.playVideo(); } }
...
function songLine(L, ln, i) {
  ...
  const open = hasYT ? `onclick="ytSeek(${t})"` : `onclick="tapSong('${L.id}',${i})"`;
...
function songView(id) {
  ...
  if (vid) h += `<div class="ytwrap" id="ytwrap"><div id="ytplayer"></div></div><p class="hint">歌詞をタップ → その秒数から再生 · 🔊 = お手本の声</p>`;
  ...
  app.innerHTML = h; window.scrollTo(0, 0);
  if (vid) { const w = document.getElementById('ytwrap'); ytThresh = (w && w.offsetHeight) || 240; ensureYT(vid); }
}
...
async function playAll(id, shadow) {
  stopAudio(); ytPause(); const my = token, L = LESSONS.find(x => x.id === id);
...
Object.assign(window, { home, lessonView, songView, reviewView, favView, tap, tapSong, playAll, toggleSlow, toggleTheme, toggleFav, ytSeek, reveal, rate });
